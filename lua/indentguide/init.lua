...
local autocmds = require "indentguide.autocmds"
local inlay_hints = require "indentguide.inlay_hints"
local indent = require "indentguide.indent"
local vt = require "indentguide.virt_text"
local scp = require "indentguide.scope"
...
---@param config indentguide.config.full
...
---@param config indentguide.config?
...
M.refresh = function(bufnr)
    ...
    for _, fn in
        pairs(hooks.get(bufnr, hooks.type.ACTIVE) --[=[@as indentguide.hooks.cb.active[]]=])
    do
    ...
    for _, fn in
        pairs(hooks.get(bufnr, hooks.type.SCOPE_ACTIVE) --[=[@as indentguide.hooks.cb.scope_active[]]=])
    do
        if not fn(bufnr) then
            scope_disabled = true
            break
    ...
    ---@type indentguide.indent_options
    local indent_opts = {
    ...
    ---@type indentguide.indent.whitespace[]
    ...
    for i, line in ipairs(lines) do
        ...
        for _, fn in
            pairs(hooks.get(bufnr, hooks.type.WHITESPACE) --[=[@as indentguide.hooks.cb.whitespace[]]=])
        do
        ...
        if scope_start and scope then
            ...
            for _, fn in
                pairs(hooks.get(bufnr, hooks.type.SCOPE_HIGHLIGHT) --[=[@as indentguide.hooks.cb.scope_highlight[]]=])
            do
        ...
        for _, fn in
            pairs(hooks.get(bufnr, hooks.type.VIRTUAL_TEXT) --[=[@as indentguide.hooks.cb.virtual_text[]]=])
        do
