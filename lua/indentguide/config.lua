...
---@type table<number, indentguide.config>
M.buffer_config = {}
...
local validate_config = function(config)
    ...
    if config.indent then
        ...
        }, config.indent, "indentguide.config.indent")
    ...
    if config.whitespace then
        ...
        }, config.whitespace, "indentguide.config.whitespace")
    ...
    if config.scope then
        ...
        }, config.scope, "indentguide.config.scope")
        ...
        if config.scope.exclude then
            ...
            }, config.scope.exclude, "indentguide.config.scope.exclude")
        end
        if config.scope.include then
            utils.validate({
                node_type = { config.scope.include.node_type, "table", true },
            }, config.scope.include, "indentguide.config.scope.include")
        end
    end
    ...
    if config.exclude then
        if config.exclude then
            ...
            }, config.exclude, "indentguide.config.exclude")
        end
    end
end

---@param behavior "merge"|"overwrite"
---@param base indentguide.config.full
---@param input indentguide.config?
---@return indentguide.config.full
local merge_configs = function(behavior, base, input)
    local result = vim.tbl_deep_extend("keep", input or {}, base) --[[@as indentguide.config.full]]
